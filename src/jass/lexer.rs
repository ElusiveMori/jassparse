...
impl<'input> CharLocations<'input> {
    pub fn new<S: AsRef<str>>(input: &'input FileMap<S>) -> CharLocations<'input> {
        CharLocations {
...
pub struct Lexer<'input, S: AsRef<str>> {
    file_map: &'input FileMap<S>,
    input: &'input str,
...
impl<'input, S: AsRef<str>> Lexer<'input, S> {
    pub fn new(file_map: &'input FileMap<S>) -> Lexer<'input, S> {
        ...
        let lexeme = match ch {
            ...
            '=' => match self.lookahead {
                Some((_, ch)) => {
                    ...
                    if lexeme != Lexeme::Assignment {
                        self.bump();
                    }

                    lexeme
                }
                None => Lexeme::Assignment,
            },
            _ => unreachable!(),
        };
    ...
    fn dec_number(
        ...
        let lexeme = if !is_real {
            Lexeme::IntegerLiteral(i32::from_str(slice).unwrap())
        } else {
    ...
    fn char_number(
        ...
        let num = BigEndian::read_i32(bytes);
...
impl<'input, S: AsRef<str>> Iterator for Lexer<'input, S> {
